//----------------------------------------------------------------------
// A barrel shifter with shift direction is controlled by an input.
...
`ifndef __VLIB_SHIFTER_V__
`define __VLIB_SHIFTER_V__
...
module vlib_shifter 
    ...
    input                            shift_dir,   // shift direction - 0: shift left; 1:shift right
    ...
    assign shift_stg[0] = (shift_amt[0]) ? (shift_dir ? {{WORD_WD{1'b0}}, in_data[WORD_CNT*WORD_WD-1:WORD_WD]} : 
                                           {in_data[(WORD_CNT-1)*WORD_WD-1:0], {WORD_WD{1'b0}}}) :
                          in_data;

generate
    for (genvar ii=1; ii<SHIFT_STG_CNT; ii++) begin: stage_ii
        assign shift_stg[ii] = (shift_amt[ii]) ? (shift_dir ? {{((1<<ii)*WORD_WD){1'b0}}, shift_stg[ii-1][WORD_CNT*WORD_WD-1:(1<<ii)*WORD_WD]} :
                                                 {shift_stg[ii-1][(WORD_CNT-(1<<ii))*WORD_WD-1:0], {((1<<ii)*WORD_WD){1'b0}}}) :
                               shift_stg[ii-1];
    end
endgenerate
